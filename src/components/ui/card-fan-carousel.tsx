...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  return (
    <div className="w-full flex flex-col items-center gap-6">
      <div
        ref={containerRef}
        className="relative w-full h-[22rem] sm:h-[26rem] md:h-[28rem] lg:h-[34rem] xl:h-[38rem] flex items-center justify-center"
      >
        {cards.map((card, index) => {
          const image = (
            <img
              ...
              className="w-full h-full object-cover rounded-2xl shadow-[0_20px_50px_-15px_rgba(0,0,0,0.35)]"
              draggable={false}
              loading="lazy"
            />
          );
          const cardClass =
            "fan-card absolute w-[9rem] h-[13rem] sm:w-[11rem] sm:h-[16rem] md:w-[13rem] md:h-[19rem] lg:w-[16rem] lg:h-[23rem] xl:w-[18rem] xl:h-[26rem] will-change-transform";
          return card.linkUrl ? (
            <a
              key={index}
              href={card.linkUrl}
